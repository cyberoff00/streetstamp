...
private struct JourneyVisibilitySheet: View {
    ...
    var body: some View {
        JourneySheetScaffold(
            ...
            VStack(spacing: 10) {
                ForEach(JourneyVisibilitySheetPresentation.optionPresentations, id: \.visibility) { option in
                    visibilityOptionCard(option)
                }
            }

            if let selectedPresentation {
                HStack(spacing: 8) {
                    Image(systemName: selectedPresentation.accentStyle == .accent ? "person.2.fill" : "lock.fill")
    ...
    private func visibilityOptionCard(_ option: JourneyVisibilitySheetOptionPresentation) -> some View {
        let isSelected = pendingVisibility == option.visibility
        let borderColor = option.accentStyle == .accent
            ? UITheme.accent.opacity(isSelected ? 0.34 : 0.12)
            : Color.black.opacity(isSelected ? 0.14 : 0.07)
        let fillColor = option.accentStyle == .accent
            ? UITheme.accent.opacity(isSelected ? 0.10 : 0.03)
            : Color.black.opacity(isSelected ? 0.04 : 0.015)
        let iconFill = option.accentStyle == .accent
            ? UITheme.accent.opacity(isSelected ? 0.15 : 0.08)
            : Color.black.opacity(isSelected ? 0.07 : 0.04)

        return Button {
            pendingVisibility = option.visibility
        } label: {
            HStack(alignment: .center, spacing: 14) {
                ZStack {
                    ...
                    Image(systemName: option.symbolName)
                        .font(.system(size: 18, weight: .semibold))
                        .foregroundColor(option.accentStyle == .accent ? UITheme.accent : UITheme.softBlack)
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(option.eyebrow)
                        ...
                        .tracking(1.0)
                        .foregroundColor(FigmaTheme.subtext.opacity(0.82))
                ...
                ZStack {
                    Circle()
                        ...
                        .background(
                            Circle()
                                .fill(isSelected ? UITheme.softBlack : Color.clear)
                        )
            ...
            .padding(.vertical, 14)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(fillColor, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    .stroke(borderColor, lineWidth: isSelected ? 1.3 : 1)
            )
        }
        .buttonStyle(.plain)
    }
