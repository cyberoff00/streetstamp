//
//  ProfileFootprintSection.swift
//  StreetStamps
//
//  The 「足迹」retrospective, inlined into the Profile scroll instead of living on
//  its own pushed page. Renders, top to bottom:
//    1. a been-style world map (visited countries filled, tap → live globe)
//    2. the caller-provided activity rings (so Profile keeps owning that data)
//    3. a timeline of recent footprint moments + a featured 重逢 card
//
//  Data/logic mirror `FootprintView` (numbers from `FootprintFactsBuilder`, names
//  resolved lazily/localized). FootprintView remains on disk but is no longer
...
struct ProfileFootprintSection<Rings: View>: View {
    ...
    /// The Profile-owned concentric rings, slotted between the map and the cards.
    @ViewBuilder var rings: () -> Rings
    ...
    var body: some View {
        ...
        .onChange(of: scenePhase) { _, phase in
            // Recompute on foreground so "X 天前" rolls over across days.
    ...
    private var cardBackground: some View {
        RoundedRectangle(cornerRadius: 28, style: .continuous).fill(FigmaTheme.card)
    }
