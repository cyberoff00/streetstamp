...
async function migrate() {
  ...
  for (const [uid, user] of Object.entries(db.users || {})) {
    try {
      ...
      insertedUsers.add(uid);
      count("users");
    } catch (e) {
      console.warn(`[migrate] skipping user ${uid} (${user.email}): ${e.message}`);
    }
    ...
    for (const friendID of user.friendIDs || []) {
      ...
      try {
        ...
        console.warn(`[migrate] skipping friendship ${uid} -> ${friendID}: ${e.message}`);
      }
    }
    ...
    for (const journey of user.journeys || []) {
      ...
      try {
        ...
        console.warn(`[migrate] skipping journey ${journey.id}: ${e.message}`);
      }
    }
    ...
    for (const card of user.cityCards || []) {
      ...
      try {
        ...
        console.warn(`[migrate] skipping city_card ${card.cityID}: ${e.message}`);
      }
    }
    ...
    for (const n of user.notifications || []) {
      ...
      try {
        ...
        console.warn(`[migrate] skipping notification ${n.id}: ${e.message}`);
      }
    }
  ...
  for (const [aid, identity] of Object.entries(db.authIdentities || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping auth_identity ${aid}: ${e.message}`);
    }
  }
  ...
  for (const [key, userID] of Object.entries(db.oauthIndex || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping oauth_index ${key}: ${e.message}`);
    }
  }
  ...
  for (const [tid, token] of Object.entries(db.emailVerificationTokens || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping email_verification_token ${tid}: ${e.message}`);
    }
  }
  ...
  for (const [tid, token] of Object.entries(db.passwordResetTokens || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping password_reset_token ${tid}: ${e.message}`);
    }
  }
  ...
  for (const [tid, token] of Object.entries(db.refreshTokens || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping refresh_token ${tid}: ${e.message}`);
    }
  }
  ...
  for (const [rid, req] of Object.entries(db.friendRequestsIndex || {})) {
    ...
    try {
      ...
      console.warn(`[migrate] skipping friend_request ${rid}: ${e.message}`);
    }
  }
  ...
  for (const [, record] of Object.entries(db.likesIndex || {})) {
    ...
    for (const likerID of record.likerIDs || []) {
      ...
      try {
        ...
        console.warn(`[migrate] skipping like ${record.journeyID}/${likerID}: ${e.message}`);
      }
    }
  }
